...
RUN --mount=type=bind,target=/azure-cli.tar.gz,source=$BUILD_DIRECTORY/azure-cli.tar.gz tar xf /azure-cli.tar.gz  --directory=/azure-cli && apk add --no-cache bash openssh ca-certificates jq curl openssl openssl1.1-compat perl git zip \
